...
- [x] Run focused API checks, dashboard check/build, website build, and available smoke checks.
...
- Final API regression harness: 41 endpoint/date/pagination/cap checks plus
  2 live Azurite checks passed. The harness is tracked under
  `tests/dashboard-api` and included in `liedertafel.slnx`; it adds no packages.
- Browser smoke: 28 checks passed against synthetic responses generated by the
  actual API functions. Verified seven-day defaults, navbar hydration, first
  keyboard target, charts, compare/reload state, invalid ranges, weekly presets,
  retained loading results, page/source/device drill-downs, pagination, expired
  cursor recovery, timelines and edge notices, masked IDs, mobile overflow,
  empty/malformed/403 responses, and 401 login redirection. No runtime errors.
- Visual review: desktop and 375px mobile screenshots inspected; removed default
  sparkline grid clutter, softened trend gridlines, and linked trend markers.
  Screenshots and temporary browser tooling are under
  `/tmp/liedertafel-insights-validation` (not application dependencies).
- Dashboard Astro + Svelte checks pass; dashboard build emits `/`, `/sessions`,
  `/403`, and `/404.html`. Corrected the 403 rewrite to its actual generated path.
  Website build passes after the privacy disclosure change. `git diff --check`
  passes. API build reports zero compiler warnings/errors.
- Environment notes: Vite reports existing plugin deprecation notices. The
  regression project's NuGet audit metadata fetch reports NU1900 (feed
  unreachable), including after an unrestricted restore; compilation and all
  assertions pass. No dependency versions or storage credentials were changed.
- Deployment remains separate: role rules were reviewed locally; production
  EasyAuth/role enforcement was not exercised or changed in the portal. API and
  dashboard must deploy together because the stats contract changed.
...
## Baseline before implementation (verified against code, not the older plans)
